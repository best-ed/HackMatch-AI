...
- Offers validation shortcuts for broken settings drafts so organizers can recover without manually hunting through every field.
...
- Filters saved runs by active cohort, final status, and attention state so review stays focused on the right snapshot set.
...
- Exports and imports participant CSV files, previews rollback after import, surfaces import diagnostics, and exports generated teams to CSV with audit-based safety gates.
...
- The admin dashboard now reads private launch/auth signals from protected server routes instead of guessing from client-side env access, so admin protection and OpenAI explanation mode report the real deployment state.
- Admin auth, security-readiness, runtime-signal, and explanation endpoints return `no-store` cache headers so session and launch state are not treated like cacheable content.
- Admin pages now warn when the current organizer session is close to expiring, reducing surprise logouts during longer review flows.
...
The participants page includes an intake quality panel for consent coverage, incomplete records, low matching signal, average profile completeness, and role concentration before organizers move into team generation. It also turns those counts into deterministic organizer nudges so the next cleanup step is obvious instead of buried in the table. Readiness quick filters let organizers jump directly to incomplete, excluded, low-signal, or duplicate participant records. Filtered batch actions let organizers move the current filtered participant set into a cohort or adjust consent/contact review states in one controlled step, then review a deterministic batch audit summary of what changed.

Duplicate review is now prioritized instead of flat. Access-token collisions are treated as the highest-risk group, email collisions stay in the middle, and weaker name-plus-institution matches stay lower. Each duplicate card also includes a deterministic resolution preview that suggests the strongest base record to keep and calls out fields that should be merged before anything is deleted.
...
The participant team page turns each access link into a team handoff with members, suggested internal roles, strengths, watch points, shared interests, shared availability, next steps, and contact details only for teammates who consented to sharing. It now also includes a handoff-readiness panel that summarizes assignment visibility, contact visibility, and coordination signal before the participant starts acting on the page.
...
The admin dashboard includes an action queue and recent activity timeline. The queue highlights the next best organizer actions from participant intake, settings health, assignment coverage, saved runs, and deployment status. The activity timeline summarizes recent participant changes, saved-run milestones, admin sign-in events, and sensitive organizer operations such as backup download/restore, participant imports, bulk participant actions, access-token rotation, delete actions, and export events.

The Team review operations history also supports focused filtering for all events, sensitive events, access events, and data-movement events. This makes it easier to isolate auth, export, share, restore, and backup actions during launch review or later incident follow-up.
...
Tests cover determinism, uniqueness, team sizes, blocked teammates, consent exclusion, advanced distribution, beginner-only penalties, locked teams, score breakdowns, CSV export, access link export, CSV import duplicate handling, CSV import validation, participant import rollback summaries, participant registration validation, participant intake quality, duplicate participant review, duplicate resolution previews, intake completeness scoring, participant intake nudges, readiness filters, privacy audits, local backup guardrails, local backup sensitivity summaries, local storage diagnostics, participant link audits, participant link handoff summaries, access token rotation and uniqueness guardrails, participant confirmation summaries, participant team briefs, participant handoff readiness, participant assignment status, participant contact privacy summaries, saved-run share previews, saved-run notes, saved-run integrity, saved-run drift comparison, final saved-run marking, cohort finalization, team review summaries, team handoff coverage, team review checklists, team balance indicators, export audits, sensitive export warnings, admin cache headers, admin session expiry warnings, admin audit filters, admin session lifecycle regressions, security headers, security readiness checks, Supabase sync posture, Supabase RLS posture checks, settings presets, settings validation, settings explanations, settings impact summaries, matching readiness evaluation, cohort health comparison, admin action queues, participant activity timelines, Supabase readiness checks, deployment readiness checks, and route smoke testing.

The core regression coverage is also organized into focused suites so future failures point to the right subsystem quickly: matching core, participant data flow, settings/readiness flow, participant team experience, and team review/platform support.
