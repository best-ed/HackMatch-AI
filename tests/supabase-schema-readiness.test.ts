...
describe("supabase schema readiness", () => {
  it("reports which persistence surfaces are remote-ready today", () => {
    ...
    expect(readiness.items.find((item) => item.label === "Saved match runs")?.status).toBe("planned");
    expect(readiness.items.find((item) => item.label === "Team review checklist")?.status).toBe("local");
  });
});
